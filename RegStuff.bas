...
Public Function LastSlash(strData As String)
    where = 4
    Do While where <> 0
        length = Len(strData)
        where = InStr(strData, "\")
        strData = Right$(strData, length - where)
        getend = getend + where 'start at location of \ + 1 next loop so it wont get stuck at first \ and loop forever
        DoEvents 'algorithm for finding the location of the last "\" in a string
        'if you know where the last "\" is, then anything to to right is the filename
        'and anything to the left is the directory name
    Loop
 LastSlash = getend
